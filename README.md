...
| --- | --- |
| P → P₁ decl | { P.cod = concat(P₁.cod, decl.cod) } |
...
| attr_list → attr_list₁ attr | { verifica_unicidade(attr_list₁.attrs, attr.nome); attr_list.attrs = merge(attr_list₁.attrs, attr.nome, attr.valor); attr_list.cod = concat(attr_list₁.cod, attr.cod) } |
...
| saida_decl → **saida de** STRING₁ **em** MEDIDA_M₁ **para** STRING₂ **em** MEDIDA_M₂ sinal_opt **;** | { verifica_existe(STRING₁.val); verifica_existe(STRING₂.val); verifica_posicao(STRING₁.val, MEDIDA_M₁.val); verifica_posicao(STRING₂.val, MEDIDA_M₂.val); temp = geratemp(); saida_decl.cod = concat(geracod(temp, "=", "saida", STRING₁.val, "+", MEDIDA_M₁.val, "→", STRING₂.val, "+", MEDIDA_M₂.val), sinal_opt.cod) } |
...
| sinal_decl → **sinal** ID **{** tempo_list **}** | { verifica_duplicado(ID.val); verifica_obrigatorios(tempo_list.tempos, "verde", "vermelho"); insere_simbolo(ID.val, tempo_list.tempos); sinal_decl.cod = concat(geracod(ID.val, "=", tempo_list.tempos), tempo_list.cod) } |
| tempo_list → tempo_list₁ tempo_item | { verifica_unicidade(tempo_list₁.tempos, tempo_item.nome); tempo_list.tempos = merge(tempo_list₁.tempos, tempo_item.nome, tempo_item.valor); tempo_list.cod = concat(tempo_list₁.cod, tempo_item.cod) } |
